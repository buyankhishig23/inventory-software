...
export default function OptionCard({optionData}) {
  ...
  return (
    ...
    {enabled?(<Link href={link} className='py-2  bg-blue-600 px-3 inline-flex items-center space-x-2 text-white rounded'>
      {linkTitle}
    </Link>):(<button className='py-2 rounded-sm bg-blue-600 px-3 inline-flex items-center space-x-2 text-white'>Идэвхжүүлэх</button>)}
  </div>
  )
}
